    ...
    AgentLoop: vi.fn().mockImplementation((options) => {
      const mockAgent = {
        run: vi.fn().mockImplementation(async (inputs, lastResponseId) => {
          ...
          const input = inputs[0]?.content?.[0]?.text || inputs[0]?.text || '';
          console.log('AgentLoop mock received input:', input);
          
          // Simulate reasoning phase
          options.onLoading(true);
          await new Promise(resolve => setTimeout(resolve, 100));
          
          if (input.includes('create a web app') || input.includes('web app')) {
            console.log('Triggering web app creation simulation');
            return simulateWebAppCreation(options);
          } else if (input.includes('fix the bug') || input.includes('bug')) {
            console.log('Triggering bug fixing simulation');
            return simulateBugFixing(options);
          } else if (input.includes('read the file') || input.includes('analyze')) {
            console.log('Triggering file reading simulation');
            return simulateFileReading(options);
          } else if (input.includes('apply patch') || input.includes('patch')) {
            console.log('Triggering patch application simulation');
            return simulateApplyPatch(options);
          } else if (input.includes('complex workflow') || input.includes('workflow')) {
            console.log('Triggering complex workflow simulation');
            return simulateComplexWorkflow(options);
          } else {
            console.log('Falling back to basic response for input:', input);
...
// Mock the model utilities to avoid API key issues
vi.mock('../src/utils/model-utils.js', () => ({
  ...
  calculateContextPercentRemaining: vi.fn().mockReturnValue(80),
  uniqueById: vi.fn().mockImplementation((items) => items),
}));
...
+    return user.id;
    ...
    
    {
      ...
      type: 'function_call_output',
      output: JSON.stringify({
        output: 'Patch applied successfully\nChanges: 3 additions, 1 deletion, 1 file changed',
        metadata: {
          exit_code: 0,
...
async function simulateApplyPatch(options: any) {
  const items = [
    ...
    
    {
      ...
      type: 'function_call_output',
      output: JSON.stringify({
        output: ' M src/components/Header.tsx\n?? patch.diff',
        metadata: {
          exit_code: 0,
          duration_seconds: 0.05
        }
...
+      </nav>
    ...
    
    {
      ...
      type: 'function_call_output',
      output: JSON.stringify({
        output: 'Patch applied successfully\nChanges: 4 additions, 1 deletion, 1 file changed',
        metadata: {
          exit_code: 0,
...
const mockConfig: AppConfig = {
  ...
  flexMode: false,
  providers: { openai: { name: 'OpenAI', baseURL: 'https://api.openai.com/v1', envKey: 'OPENAI_API_KEY' } },
...
describe('Comprehensive CLI Simulation', () => {
  ...
  it('should simulate web app creation workflow', async () => {
    ...
    // Wait for agent initialization and simulation to complete
    await new Promise(resolve => setTimeout(resolve, 3000));

    const frame = lastFrame();
    expect(frame).toContain('Web app created successfully');
  ...
  it('should simulate bug fixing with code analysis', async () => {
    ...
    // Wait for agent initialization and initial simulation steps
    await new Promise(resolve => setTimeout(resolve, 2000));

    const frame = lastFrame();
    // Check for evidence that the bug fixing simulation is working
    expect(frame).toContain('analyze');
    expect(frame).toContain('bug');
    expect(frame).toContain('error logs');
  });
  ...
  it('should simulate function calls with realistic outputs', async () => {
    ...
    expect(frame).toContain('File created successfully');
    expect(frame).toContain('Web app created successfully');
  });
}); 